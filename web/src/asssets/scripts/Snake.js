...
export class Snake extends AcGameObect {
    constructor(info, gamemap) {
        ...
        this.step = 0; // 表示回合数
        this.eps = 1e-2; // 允许的最小误差

        this.eyes_direction = 0;
        if (this.id === 1) this.eyes_direction = 2; // 左下角的蛇初始朝上，右上角的蛇初始朝下
    ...
    check_tail_increasing() { // 检测当前回合，蛇尾是否增加
    ...
    update_move() {
        ...
        if (distance < this.eps) { // 走到目标点了
            ...
            this.next_cell = null;
            this.status = "idle"; // 走完停下来了

            if (!this.check_tail_increasing()) { //蛇不变长了
                this.cells.pop();
            }
        } else {
    ...
    render() {
        ...
        ctx.fillStyle = "Black";
        for (let i = 0; i < 2; i++) {
            const eye_x = (this.cells[0].x + this.eye_dx[this.eyes_direction][i] * 0.15) * L;
            const eye_y = (this.cells[0].y + this.eye_dy[this.eyes_direction][i] * 0.15) * L;
            ctx.beginPath();
